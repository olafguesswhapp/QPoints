...
exports.defineNav = function (req, res) {
    ...
    switch(currentPath) {
        ...
        case 'admin':
            res.locals.navAdmin = 'class="active"';
            res.locals.current = 'clients';
            if (current2ndPath == 'user') {res.locals.current = 'user';}
            if (current2ndPath == 'programme') {res.locals.current = 'programms';}
        default:
    } // swith
    return;
}; // defineNav
